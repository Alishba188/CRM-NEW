  ...
  try {
    ...
    if (tableDefinition && tableDefinition.name) {
      try {
        ...
        if (err.original && (err.original.code === "ER_BAD_FIELD_ERROR" || err.original.code === "SQLITE_ERROR")) {
          console.warn("⚠️ Legacy leads.name column not found or already migrated.");
        } else {
          console.warn("⚠️ Non-critical schema warning on leads.name:", err.message);
        }
      }
    }

    const { Lead } = require("./models");
    if (Lead && tableDefinition) {
      for (const [columnName, attribute] of Object.entries(Lead.rawAttributes)) {
        if (!tableDefinition[columnName]) {
          try {
            ...
            if (err.original && (err.original.code === "ER_DUP_FIELDNAME" || err.original.code === "SQLITE_ERROR")) {
              console.warn(`⚠️ Column leads.${columnName} already exists, skipping.`);
              continue;
            }
            console.warn(`⚠️ Could not add column leads.${columnName}:`, err.message);
          }
        }
      }
    }
  } catch (globalErr) {
    console.warn("⚠️ Leads schema check bypassed:", globalErr.message);
  }
}
  ...
  try {
    ...
    if (tableDefinition.status) {
      try {
        ...
        console.warn("⚠️ Task status normalization skipped or already updated.");
      }
    }
    ...
    console.warn("⚠️ Task schema check bypassed:", globalErr.message);
  }
}
